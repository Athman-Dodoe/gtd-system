...
export default function ChangePasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Sign out and redirect to login — the new session will have mustChangePassword = false
      setTimeout(async () => {
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950 text-slate-100 px-4">
      ...
      <div className="relative z-10 w-full max-w-md p-8 bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-2xl shadow-2xl">
        ...
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          <h1 className="text-xl font-bold text-white mt-1">Set Your Password</h1>
          <p className="text-xs text-slate-400 mt-2 max-w-xs">
            This is your first login. You must set a personal password before accessing the system.
          </p>
        </div>
        ...
        {success ? (
          <div className="p-4 bg-emerald-950/50 border border-emerald-500/30 text-emerald-200 text-sm rounded-lg text-center space-y-1">
            ...
            <p className="text-xs text-slate-400">Signing you out to apply your new credentials…</p>
          </div>
        ) : (
        ...
        <div className="mt-6 text-center border-t border-slate-800/60 pt-5">
          <p className="text-[10px] text-slate-600">
            Your new password is stored securely and cannot be recovered. Contact the DSG if you need a reset.
          </p>
        </div>
      </div>
    </div>
